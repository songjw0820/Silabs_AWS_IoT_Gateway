...
import {
  RefreshControl, StyleSheet, Text, View, FlatList, ActivityIndicator, SafeAreaView, TouchableWithoutFeedback, Keyboard,
  TouchableOpacity, Image, Modal, Platform, Alert, PermissionsAndroid, TextInput, KeyboardAvoidingView,
...
import {
  getDevices, setBleManager, addBleDevice, removeBleDevice, getDeviceTypes, clearCurrentData, isScreenLoading,
  registerDevices, getGrowAreaCounts, getRecentData, removeBleDevicefromDevice, deleteDeviceResponse, deleteDevice
...
class Devices extends Component {
  ...
  async componentDidAppear() {
    console.log('in compomemt will mount--------');
    this._onRefresh();
    this.visible = true;
    this.forceUpdate();
  ...
  _onRefresh = async () => {
    ...
    AsyncStorage.getItem('accessToken').then(async (authToken) => {
      ...
      console.log('error in get async call', error);
      this.setState({ refreshing: false })
    });
  }
  ...
  getCurrentData = (data) => {
    ...
    if (data) {
      if (data.length !== 0) {
        data.map((listItem, i) => {
          console.log('deviceHid', listItem.deviceHId);
  ...
  redirectToLEDGrops() {
    console.log('this.growareaID--------------->', this.growAreaId);
    console.log('going to led groups----------->, ', this.props.gateway);
  ...
  showDeviceDiscoveryModal(visible, inBackground, keepProvisionCallback, showAlert) {
    ...
    if (visible) {
      ...
      if (!inBackground) {
        this.setState({ deviceDiscoveryModalVisible: true });
        console.log('flag for modal', this.state.deviceDiscoveryModalVisible);
      }
      const subscription = this.props.bleManager.onStateChange((state) => {
        console.log('state in phone', state);
        if (state === 'PoweredOn') {
          if (!inBackground) {
            this.setState({ deviceDiscoveryModalVisible: true });
            console.log('flag for modal', this.state.deviceDiscoveryModalVisible);
  ...
  onRegisterDevicesClick() {
    ...
    if (this.deviceProvisionChar && this.device) {
      ...
      if (this.onTimeRegistredDevices === 0) {
        console.log('appp this.onTimeRegister', this.onTimeRegistredDevices);

        this.discoonecFromCancel = true;
      }
      ...
      if (this.onTimeRegistredDevices !== 0) {
        ...
        setTimeout(() => {
          ...
          if (!this.state.isRegistrationProcessCompleted) {
            this.setState({ deviceRegistrationModalVisible: false, isRegistrationProcessCompleted: true })
            this._onRefresh()
  ...
  stopDeviceScanUnsubscribeNotification(keepProvisionCallback) {
    ...
    if (this.deviceDiscoverChar && this.device) {
      console.log(' step 1 per write ma gayu ');
      this.discoonecFromCancel = true;
      ...
      if (!keepProvisionCallback) {
        if (!this.discoonecFromCancel) {
          console.log(' step 2 per write ma gayu ');
          this.discoonecFromCancel = true;
        }
                ...
                [
                  ...
                  },
                  {
                    text: 'OK', onPress: () => {
                      ...
                      this.setState({ errorCode: 1 });
                      this.stopDeviceScanUnsubscribeNotification(false);
                    }
                  },
                ],
                { cancelable: false },
  ...
  checkForGatewayConnection = (inBackground) => {
    ...
    if (this.growAreaId) {
      ...
      if (this.props.bleDevices[this.GatewayMacID]) {
        ...
        console.log("Device found in redux..");
    ...
    this.discoverCharSubscription = deviceProvisionChar.monitor((error, characteristic) => {
      ...
      if (error) {
        if (this.state.errorCode === 1) {
          ...
          this.setState({ errorCode: 1 });
          if (this.state.deviceRegistrationModalVisible || this.state.deviceDiscoveryModalVisible) {
            this.setState({ deviceRegistrationModalVisible: false, deviceDiscoveryModalVisible: false, }, (() => {
              this.visible = true;
              setTimeout(() => {
        ...
        if (characteristic.value) {
          ...
          console.log('resopone is null while monitoring characteristic')
        }
      }
  ...
  handleProvisionCallbackNotifications = (deviceProvisionCallbackChar) => {
    ...
    this.provisionCallbackCharSubscription = deviceProvisionCallbackChar.monitor((error, characteristic) => {
      ...
      this.setState({ showCancelButton: false }); // hide pre defined cancel button.
      if (error) {
        if (this.state.errorCode === 1) {
        ...
        if (characteristic.value) {
          ...
          console.log(' null value for provison callback ');
    ...
    if (this.props.bleManager) {
      ...
      this.props.bleManager.startDeviceScan(null, null, (error, device) => {
        ...
        if (device.name && device.id) {
          ...
          console.log('condition', ((device.name === this.props.selectedGrowArea.macId) || (device.id === this.props.selectedGrowArea.macId)), typeof (device.name), this.props.selectedGrowArea.macId);
          console.log('index    ', device.name.indexOf(this.props.selectedGrowArea.macId));
  ...
  handleConnectivityNotifications(connectivityChar) {
    ...
    if (this.deviceConectivityCharFound) {
      this.connectivitySubscription = connectivityChar.monitor((error, response) => {
        if (error) {
          console.log('error453544', error, this.state.errorCode === 1);
          if (this.state.errorCode === 1) {
            console.log("Error:" + error.message + "\nErrorCode:" + error.errorCode + '\nDiscoverDeviceProvisionCallbackNotificationSubscription');
          } else {
            if (this.state.deviceRegistrationModalVisible || this.state.deviceDiscoveryModalVisible) {
          ...
          <View style={styles.listItem}>
            ...
            <View style={{ justifyContent: 'space-evenly', flexDirection: 'row', marginTop: 10, alignItems: 'center', width: '100%' }}>
              <View style={{ flexDirection: 'column', alignItems: "center", backgroundColor: '#737373' }}>
                ...
                <Text style={{ color: '#fff', fontWeight: 'bold', paddingHorizontal: 12, paddingBottom: 5 }}>Live Chart</Text>
              </View>
  ...
  pickerActivity() {
    alert('alert')
    this.setState({ ch1set: false });
    console.log('log', this.state.ch1set);
  }
      ...
      if (listData.length !== 0) {
        if (this.props.currentData) {
          if (this.props.currentData.length === 0) {
            ...
            console.log('no device condition........, werwefwscd');
        ...
        if (this.state.refreshing) {
          console.log('no device condition........, refreshing', this.state.refreshing);
          devicesList = <View style={styles.activityIndicator}><ActivityIndicator size="large" color={Constant.PRIMARY_COLOR} /></View>;
        } else {
          console.log('no device condition........, out refreshing', this.state.refreshing);
          devicesList = (
                ...
                <View style={styles.deviceItem}>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <View style={{ width: '40%', paddingRight: 10, flexDirection: 'column' }}>
                      ...
                      <Text numberOfLines={1} style={{ fontSize: 12, fontWeight: "bold", width: 180 }}>{info.item.eui64}</Text>
                    </View>
                  ...
                  </View>
                  {
                    ...
                    <View>
                      ...
                      <View style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "space-evenly" }}>
                        <Text> CH 1 : </Text>
                        <View style={{ marginLeft: 2, width: "35%", borderBottomWidth: 1, borderColor: Constant.LIGHT_SILVER_COLOR }}>
                          {console.log((this.state.discoveredDevices[info.item.eui64].ledconfigurationflag === 0))}
                          <Picker
                            ...
                            disabled={(this.state.discoveredDevices[info.item.eui64].ledconfigurationflag === 0) ? false : true}   // disabled={true}
                            style={(this.state.discoveredDevices[info.item.eui64].ledconfigurationflag) === 0 ? { height: 30 } : { height: 30, backgroundColor: '#DCDCDC' }}
                        ...
                        <View style={{ width: "35%", borderBottomWidth: 1, borderColor: Constant.LIGHT_SILVER_COLOR }}>
                          <Picker
                            ...
                            disabled={(this.state.discoveredDevices[info.item.eui64].ledconfigurationflag === 0) ? false : true} f
                            style={(this.state.discoveredDevices[info.item.eui64].ledconfigurationflag) === 0 ? { height: 30 } : { height: 30, backgroundColor: '#DCDCDC' }}
                            textStyle={{ fontSize: 15 }}
                      ...
                      <View style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "space-evenly" }}>
                        ...
                        <View style={{ width: "35%", borderBottomWidth: 1, borderColor: Constant.LIGHT_SILVER_COLOR }}>
                          <Picker
                            ...
                            disabled={(this.state.discoveredDevices[info.item.eui64].ledconfigurationflag === 0) ? false : true}
                            onValueChange={(itemValue) => {
                              let discoveredDevices = this.state.discoveredDevices;
                              if (!discoveredDevices[info.item.eui64].ledconfiguration) discoveredDevices[info.item.eui64].ledconfiguration = {}
        ...
        <View style={styles.detailBlock}>
          ...
          <View style={{ flexDirection: 'row', alignItems: "center", justifyContent: 'space-evenly', marginTop: 10, marginBottom: 10, marginLeft: 10 }}>
            <TouchableOpacity style={[styles.roundButton, { backgroundColor: Constant.PRIMARY_COLOR, width: 90, height: 35 }]} onPress={() => {
              ...
              this.setState({ callbackRegistredDevices: 0, showCancelButton: true, discoveredDrvices: {} })
            }}
            >
...
mapStatesToProps = state => {
  ...
  return {
    ...
    isDeviceDetailsFetched: state.device.loading
  }
};

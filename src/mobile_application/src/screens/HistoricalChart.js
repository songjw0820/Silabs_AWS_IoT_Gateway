...
class HistoricalChart extends Component {
    ...
    Validation(property, date, toDate, fromDate) {
        ...
        if (property != "" && date != "") {
            ...
            this.props.getHistoricalData(toDate, fromDate, this.props.deviceHid, property, token);
    ...
    render() {
        if (this.props.retry401Count === 20 && !this.state.enabled401) {
            this.setState({ isFromDateTimePickerVisible: false, isToDateTimePickerVisible: false, enabled401: true })
        }
            ...
            <View style={styles.mainContainer}>
                ...
                <View style={{ margin: 10, marginTop: 15, elevation: 5, backgroundColor: "#fff", shadowColor: "black", shadowOpacity: 1 }}>
                    ...
                    <Picker
                        ...
                        enabled={this.props.deviceProperty.length !== 0}
                        style={{ width: '100%' }}
...
const mapStateToProps = state => {
    const { historicalData, deviceProperty, loading } = state.device;
    const { retry401Count } = state.auth;

    return { historicalData, deviceProperty, loading, retry401Count };
};

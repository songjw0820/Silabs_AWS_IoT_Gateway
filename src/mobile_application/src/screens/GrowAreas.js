  ...
  writeCharacteristics(characteristic, payload, chunkSize, info) {
    ...
    characteristic.writeWithResponse(Base64.btoa(Constant.BLE_PAYLOAD_SUFFIX), info + '$').catch(error => {
      ...
      if (this.errorCode === 1) {
        ...
        Promise.resolve(this.props.onRegisterGateway(this.props.internalCloudePayload, this.bleDevice)).then(() => {
          this.bleDevice = null;
        });
      }
    });
                    ...
                    [
                      ...
                      },
                      {
                        text: 'Delete', onPress: () => {
                          ...
                          this.props.onDeleteGateway(this.state.token, item.id);
                          alert(item.grow_area_name + ' Deleted');
                          this._onRefresh();
                        }
                      },
                    ],
        ...
        <View style={styles.listContainer}>
          <View style={styles.titleContainer}>
            ...
            <TouchableOpacity style={[styles.roundButton, styles.addNewButton]} onPress={async () => {
              ...
              this.alreadyRegistredGateways = await this.alreadyRegistredGateway(this.props.alreadyProvisionedGateway)
